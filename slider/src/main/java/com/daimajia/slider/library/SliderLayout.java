...
public class SliderLayout extends RelativeLayout{
    ...
    public void setPresetTransformer(String transformerName){
        Random random;
        if (transformerName.equals("Random")){
            random = new Random();
    ...
    public <T extends BaseSliderView> void replaceSliderAt(int position, T slider){
        if(getRealAdapter()!=null){
            getRealAdapter().replaceSliderAt(position, slider);
            mViewPager.setCurrentItem(mViewPager.getCurrentItem(),false);
        }
    }
     ...
     * @param position
     */
    public void setCurrentPosition(int position, boolean smooth) throws IllegalStateException{
        if (getRealAdapter() == null)
            throw new IllegalStateException("You did not set a slider adapter");
        if(position >= getRealAdapter().getCount()){
            throw new IllegalStateException("Item position is not exist " + position + " " + getRealAdapter().getCount());
        }
